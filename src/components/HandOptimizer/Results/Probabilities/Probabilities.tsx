...
const operators = [
  { label: '<', value: '<', compare: (score: number, threshold: number) => score < threshold },
  { label: '<=', value: '<=', compare: (score: number, threshold: number) => score <= threshold },
  { label: '=', value: '=', compare: (score: number, threshold: number) => score === threshold },
  { label: '>=', value: '>=', compare: (score: number, threshold: number) => score >= threshold },
  { label: '>', value: '>', compare: (score: number, threshold: number) => score > threshold },
...
export const Probabilities = ({ hands, isMyCrib }: Props) => {
  ...
  const visibleStats = useMemo(() => new Set<StatKey>(['weightedDistribution']), []);

  const compareFn = operators.find((op) => op.value === operator)?.compare
  ...
  const getCumulativeProb = (
    ...
    return Object.entries(dist).reduce((acc, [scoreStr, weightedDistribution]) => {
